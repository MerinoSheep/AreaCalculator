...
def runGraph():
    ...
    if statusS and statusE and status_n:
        if(ENDRANGE > STARTRANGE):
            if(True):
                error_var.set('')
                if(not (graph.draw(STARTRANGE, ENDRANGE, N,
                            DRAWTYPE, fx, menu.get()))):
...
#CheckButton
isRiemannCheck = Checkbutton(
    window, text="Riemann", command=isRiemannCheck)
isTrapezoidCheck = Checkbutton(
    window, text="Trapezoid", command=isTrapezoidCheck)
#Initialized 
